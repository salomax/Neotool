...
open class AbacPolicyService(
    ...
    fun findById(id: UUID): AbacPolicy? {
        return abacPolicyRepository.findById(id).orElse(null)?.toDomain()
    }

    fun findByName(name: String): AbacPolicy? {
        return abacPolicyRepository.findByName(name).orElse(null)?.toDomain()
    }

    fun findAll(): List<AbacPolicy> {
        return abacPolicyRepository.findAll().map { it.toDomain() }
    }

    fun findActivePolicies(): List<AbacPolicy> {
        return abacPolicyRepository.findActivePolicies().map { it.toDomain() }
    }
