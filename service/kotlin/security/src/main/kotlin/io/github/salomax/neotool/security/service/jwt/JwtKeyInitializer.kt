...
import io.github.salomax.neotool.common.security.key.JwtKeyGenerator
...
import io.github.salomax.neotool.common.security.key.WritableKeyManager
...
class JwtKeyInitializer(
     ...
     */
    private fun initializeKeysWithVaultLock(
        keyId: String,
     ...
     */
    private fun initializeKeysWithFileLock(
        keyId: String,
        keyManager: KeyManager,
    ) {
        fileLock.withLock {
    ...
    private fun acquireLockWithRetry(lockPath: String): VaultClient.VaultLock? {
        ...
        while (attempt < jwtConfig.lockRetryAttempts) {
            ...
            if (attempt < jwtConfig.lockRetryAttempts) {
                logger.debug {
                    "Lock acquisition failed, retrying in ${jwtConfig.lockRetryDelaySeconds}s (attempt $attempt/${jwtConfig.lockRetryAttempts})"
                }
    ...
    private fun generateAndStoreKeys(
        ...
        val success =
            if (keyManager is WritableKeyManager) {
