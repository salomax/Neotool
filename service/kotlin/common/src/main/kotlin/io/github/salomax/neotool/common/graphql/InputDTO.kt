...
object GraphQLValidations {
    ...
    fun min(
        value: Long,
        message: String = "Value must be >= $value",
    ) = Min(value = value, message = message)

    @JvmStatic
    fun max(
        value: Long,
        message: String = "Value must be <= $value",
    ) = Max(value = value, message = message)

    @JvmStatic
    fun size(
        min: Int,
        max: Int,
        message: String = "Size must be between $min and $max",
    ) = Size(min = min, max = max, message = message)
}
