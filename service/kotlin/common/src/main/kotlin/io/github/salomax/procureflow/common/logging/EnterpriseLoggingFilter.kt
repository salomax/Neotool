...
class EnterpriseLoggingFilter : HttpServerFilter {
    ...
    // Paths to exclude from logging (monitoring/health endpoints)
    ...
    override fun doFilter(
        ...
        // Log request entry only if not excluded
    ...
    private fun shouldLogRequest(request: HttpRequest<*>): Boolean {
    ...
    private fun logRequestEntry(
        ...
        logger.info {
            "HTTP Request: $method $path - Client: $clientIp - User-Agent: $userAgent - " +
                "Size: $contentLength - CorrelationId: $correlationId"
        }
    }
}
