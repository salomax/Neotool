...
class FileKeyManager(
    ...
    override fun getSecret(keyId: String): String? {
        ...
        val secret = jwtConfig.secret.takeIf { it.isNotBlank() && it != "change-me-in-production-use-strong-random-secret-min-32-chars" }
        if (secret != null) {
            cachedSecret = secret
    ...
    fun storePrivateKey(keyId: String, privateKey: PrivateKey): Boolean {
        return try {
            ...
            file.writeText(privateKeyPem, java.nio.charset.StandardCharsets.UTF_8)
    ...
    fun storePublicKey(keyId: String, publicKey: PublicKey): Boolean {
        return try {
            ...
            val file = java.io.File(filePath)
            file.parentFile?.mkdirs()
            file.writeText(publicKeyPem, java.nio.charset.StandardCharsets.UTF_8)
     ...
     * @return true on success, false on failure
     */
    override fun storeKeyPair(keyId: String, privateKey: PrivateKey, publicKey: PublicKey): Boolean {
        return try {
