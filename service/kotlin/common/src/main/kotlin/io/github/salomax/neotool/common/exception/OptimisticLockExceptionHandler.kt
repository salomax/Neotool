...
@Produces
@Requires(
    classes = [StaleObjectStateException::class, StaleStateException::class, OptimisticEntityLockException::class],
)
class OptimisticLockExceptionHandler : ExceptionHandler<Exception, HttpResponse<Map<String, Any>>> {
    ...
    override fun handle(
        ...
        val isOptimisticLockException =
            exception is StaleObjectStateException ||
        ...
        if (!isOptimisticLockException) {
            ...
            logger.error(
                "OptimisticLockExceptionHandler caught non-optimistic-lock exception: " +
