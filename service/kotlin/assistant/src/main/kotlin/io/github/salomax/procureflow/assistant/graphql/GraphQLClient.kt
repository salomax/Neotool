package io.github.salomax.neotool.assistant.graphql
...
class GraphQLClient(
    ...
    private val baseUrl: String by lazy {
        ...
        val url = URL(fullUrl)
        val portSuffix = if (url.port != -1 && url.port != url.defaultPort) ":${url.port}" else ""
        val base = "${url.protocol}://${url.host}$portSuffix"
    ...
    private val graphqlPath: String by lazy {
        ...
        val finalPath =
            if (!fullUrl.endsWith("/graphql") && !fullUrl.endsWith("/graphql/")) {
                if (fullUrl.endsWith("/")) {
                    "${fullUrl}graphql"
                } else {
                    "$fullUrl/graphql"
    ...
    suspend fun execute(request: GraphQLRequest): GraphQLResponse {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val httpRequest =
                    HttpRequest.POST(graphqlPath, request)
