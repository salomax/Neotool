...
status: current
version: 1.0.0
tags: [terraform, infrastructure-as-code, k3s, deployment]
ai_optimized: true
search_keywords: [terraform, iac, infrastructure, k3s, modules, vpc, storage]
related:
...
> **Purpose**: Guide for using Terraform to manage Neotool infrastructure including K3S clusters, VPC networking, and storage.
...
│   ├── prod/         # Production VPC (AWS/GCP/Azure)
│   └── hostinger/    # Hostinger VPC
...
module "networking" {
  ...
  provider_type = "aws"  # or gcp, azure, hostinger, local
...
**Note**: For Hostinger, VPC networking is managed by Hostinger. The networking module is not required.
...
Multi-node K3S in VPC (AWS/GCP/Azure):
...
### Hostinger VPC Environment

Multi-node K3S on Hostinger VPS:

```bash
cd infra/terraform/environments/hostinger
cp terraform.tfvars.example terraform.tfvars
# Edit terraform.tfvars with Hostinger API token and configuration
terraform init
terraform plan
terraform apply
```

**See**: [Hostinger VPC Runbook](./hostinger-vpc-runbook.md) for detailed instructions.
...
- `provider_type`: Cloud provider (aws, gcp, azure, hostinger, local)
...
variable "provider_type" {
  validation {
    condition     = contains(["aws", "gcp", "azure", "hostinger", "local"], var.provider_type)
    error_message = "Provider must be aws, gcp, azure, hostinger, or local"
  }
}
```
...
The Hostinger Terraform provider enables direct VPS provisioning:
...
**Key Features**:
- Direct VPS instance provisioning
- Post-install script support
- SSH key management
- Integrated with K3S module

**See**: [Hostinger Setup Guide](./hostinger-setup-guide.md) for detailed configuration.
