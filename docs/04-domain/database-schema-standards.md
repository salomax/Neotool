...
### Rule: CHAR(1) Type Mapping

**Rule**: Regardless of database type being `CHAR(1)`, in Kotlin code ALWAYS treat it as `String`, never as `Char`.

**Rationale**: 
- PostgreSQL returns `CHAR(1)` as String in JDBC results
- Consistent handling across all database string types
- Avoids type conversion issues in native queries
- Simplifies code by using one string type

**Example**:
```kotlin
// ✅ Correct
@Column(name = "tipo_consolidacao", length = 1)
open var tipoConsolidacao: String? = null  // String, not Char

// In native query result mapping:
val tipoConsolidacao = (row[columnIndex] as? String)?.firstOrNull()  // Extract Char if needed

// ❌ Incorrect
@Column(name = "tipo_consolidacao", length = 1)
open var tipoConsolidacao: Char? = null  // Don't use Char type

// In native query result mapping:
val tipoConsolidacao = row[columnIndex] as? Char  // Don't cast directly to Char
```

**Note**: If you need a `Char` value from a `CHAR(1)` column, extract it from the String using `.firstOrNull()` or similar methods.
...
- **PostgreSQL < 18**: Migration must install `pg_uuidv7` extension
...
```sql
-- ✅ Correct - PostgreSQL 18+ (no extension needed)
SET search_path TO app, public;

CREATE TABLE IF NOT EXISTS app.users (
    id UUID PRIMARY KEY DEFAULT uuidv7(),  -- Built-in in PostgreSQL 18+
    -- other columns
);
```
...
**Example (PostgreSQL < 18)**:
```sql
-- ✅ Correct - PostgreSQL < 18 (extension required)
SET search_path TO app, public;

CREATE EXTENSION IF NOT EXISTS pg_uuidv7;  -- Required for PostgreSQL < 18
CREATE TABLE IF NOT EXISTS app.users (
