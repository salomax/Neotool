/**
 * Page Object Model for Role Search
 */
...
export class RoleSearch {
  ...
  private getSearchInput() {
    return this.page.locator(SELECTORS.roleSearch).locator('input');
  }
  ...
  async search(query: string): Promise<void> {
    ...
    // Also wait for role list to be visible (might be empty, but should be present)
    await this.page.waitForSelector(SELECTORS.roleList, { timeout: 10000 }).catch(() => {
  ...
  async clear(): Promise<void> {
    ...
    // Also wait for role list to be visible (reset state)
    await this.page.waitForSelector(SELECTORS.roleList, { timeout: 10000 }).catch(() => {
      // Table might not exist if empty, so just wait for search to be visible
    });
  }
  ...
  async verifyVisible(): Promise<void> {
    await expect(this.page.locator(SELECTORS.roleSearch)).toBeVisible();
  }
  ...
  async waitForSearchComplete(): Promise<void> {
    ...
    // Also wait for role list to be visible
    await this.page.waitForSelector(SELECTORS.roleList, { timeout: 10000 }).catch(() => {
      // Table might not exist if empty, so just wait for search to be visible
    });
  }
