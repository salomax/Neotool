...
This runbook provides step-by-step instructions for deploying Neotool infrastructure to Kubernetes.
...
3. Manually run vault-init job:
   ```bash
   kubectl create job --from=cronjob/vault-jwt-init vault-jwt-init-manual -n neotool-security
   ```
